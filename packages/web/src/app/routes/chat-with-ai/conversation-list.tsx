...
export function ConversationList({
  ...
  const { today, yesterday, older } = createMemo(() => {
  ...
  const renderGroup = (label: string, items: ChatConversation[]) => {
    ...
    return (
      <div class="mb-2 flex flex-col gap-px">
        ...
        <Show when={!isCollapsed}>{items.map((conv, index) => (
                                                <button
                                                  type="button"
                                                  key={conv.id}
                                                  class={cn(
  ...
  return (
    <div class="flex flex-col h-full shrink-0" style={{ width: '220px' }}>
      ...
      <div class="flex-1 relative min-h-0">
        ...
        >
          <Show when={isLoadingConversations} fallback={<Show when={conversations.length === 0} fallback={<>
                                                                  ...
                                                                  <div class="space-y-2 px-2 pt-2">
                                                                    <For each={Array.from({ length: 4 })}>{(_, i) => (
                                                                                                                          <Skeleton key={i} class="h-7 w-full rounded-md" />
                                                                                                                        )}</For>
                                                                  </div>
                                                                </Show>
        </div>
