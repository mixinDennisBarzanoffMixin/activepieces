...
export const ExploreTemplateCard = (props: TemplateCardProps) => {
  const displayTags = () => props.template.tags.slice(0, 2);
  const hasFlows = () =>
    props.template.flows && props.template.flows.length > 0;
  const { gradient } = useGradientFromPieces(
    hasFlows() ? props.template.flows![0]?.trigger : undefined,
  );

  return (
    <Card
      onClick={() => props.onTemplateSelect(props.template)}
      variant={'interactive'}
    ...
    >
      <CardContent class="py-5 px-4 flex flex-col gap-1 flex-1 min-h-0">
        ...
        <p class="text-muted-foreground text-sm line-clamp-3 mt-1 flex-shrink-0">
          <Show
            when={props.template.summary}
            fallback={<span class="italic">{t('No summary')}</span>}
          >
        ...
        <div class="h-8 flex gap-2 flex-wrap overflow-hidden mt-1 flex-shrink-0">
          <Show when={displayTags().length > 0} fallback={<div />}>
            <For each={displayTags().slice(0, 1)}>
              {(tag, index) => (
                <TagWithBright
                  index={index()}
                  prefix={t('Save')}
      ...
      >
        <Show when={hasFlows() && props.template.flows![0]?.trigger}>
          <PieceIconList
