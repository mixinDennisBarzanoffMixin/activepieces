  ...
  try {
    ...
    if (auth.access.principal.kind !== 'user') {
      throw new WorkosAuthError(
        'machine principals cannot open an Activepieces user session',
        403,
      );
    }
    return {
    ...
    const auth =
      err instanceof WorkosAuthError
        ? err
        : new WorkosAuthError('WorkOS authorization failed', 503);
...
export async function resolveVeritlyPrincipal({
  request,
  reply,
  log,
}: ResolvePrincipalParams): Promise<UserPrincipal | undefined> {
  const values = headers(request, 'authorization');
  if (values.length === 0) return;
  if (values.length !== 1)
    throw new WorkosAuthError('authorization header is invalid');
  const auth = values[0]!;
  if (!auth.startsWith('Bearer ') || !isWorkosToken(auth.slice(7))) return;
  const session = await resolveVeritlySession({
    request,
    reply,
    setStatus: false,
  });
...
export async function getVeritlySessionResponse({
  request,
  reply,
  log,
}: ResolvePrincipalParams) {
...
export async function getVeritlyProject(
...
async function getPlatform(params: GetPlatformParams) {
  ...
  if (id || owner) {
    if (!id || !owner)
      throw new WorkosAuthError(
        'Veritly Activepieces platform configuration is incomplete',
        503,
      );
    const platform = await platformService(params.log).getOneOrThrow(id);
    if (platform.ownerId !== owner) {
      throw new WorkosAuthError(
        'Veritly Activepieces platform owner is invalid',
        503,
      );
    }
    return platform;
  ...
  if (system.get(AppSystemProp.ENVIRONMENT) !== ApEnvironment.DEVELOPMENT) {
    throw new WorkosAuthError(
      'Veritly Activepieces platform is not provisioned',
      503,
    );
  }

  const platforms = await platformService(params.log).getAll();
  if (platforms.length !== 1) {
    throw new WorkosAuthError(
      'Local Veritly Activepieces requires exactly one seeded platform',
      503,
    );
  }
  return platforms[0]!;
...
async function getProject(params: GetProjectParams): Promise<Project> {
  ...
  if (existing) {
    ...
    if (
      access.projectId !== params.veritlyProjectId ||
...
function role(value: ProjectRole) {
  ...
  throw new WorkosAuthError(
    'automation grants cannot open an Activepieces user session',
    403,
  );
}
