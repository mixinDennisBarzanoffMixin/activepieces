...
export async function claimOfficeJob(jobId: string, token: string) {
  return databaseConnection().transaction(async (manager) => {
    ...
    const expires = row.leaseExpiresAt
...
export async function finishOfficeJob(
  jobId: string,
  token: string,
  outcome: JobOutcome,
) {
  if (outcome === JobOutcome.COMPLETED || outcome === JobOutcome.FAILED) {
    const rows = postgresReturningRows<{ runId: string }>(
      await databaseConnection().query(
        ...
        [
          jobId,
          token,
          outcome === JobOutcome.COMPLETED ? 'succeeded' : 'failed',
        ],
      ),
    );
