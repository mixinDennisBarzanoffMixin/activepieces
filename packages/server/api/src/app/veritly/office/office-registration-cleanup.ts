...
export const officeRegistrationCleanup = {
  ...
  async wake() {
    ...
    running = drain(log)
      .catch((err: unknown) =>
        log.error({ err }, 'Office registration cleanup sweep failed'),
      )
      .finally(() => {
        running = undefined;
      });
...
async function drain(log: FastifyBaseLogger): Promise<void> {
  ...
  await Promise.all(
    rows.map((row) =>
      clean(log, row).catch(async () => {
        await fail(row);
        log.warn(
          { registrationId: row.id, attempts: row.cleanupAttempts },
          'Office registration cleanup deferred',
        );
      }),
    ),
  );
...
async function clean(log: FastifyBaseLogger, row: CleanupRow) {
  ...
  if (
    created.kind === 'error' ||
    created.value.verification !== 'office_managed'
  ) {
  ...
  await renew(row);
  const dropped = await removeDestination({
    ...ctx,
    destination: destinationId,
  });
  if (
    dropped.kind === 'error' &&
    dropped.error.code !== 'destination_not_found'
  ) {
    throw new OfficeRegistrationCleanupError();
  }
...
async function claim(): Promise<CleanupRow[]> {
  ...
  return databaseConnection().transaction(async (manager) =>
    postgresReturningRows<CleanupRow>(
      await manager.query(
        `WITH next AS (
             SELECT id
             FROM veritly_office_registration
...
async function attach(
  ...
  if (row.destinationId && row.destinationId !== destinationId)
    throw new OfficeRegistrationCleanupError();
  if (row.registrationId && row.registrationId !== registrationId)
    throw new OfficeRegistrationCleanupError();
  const result = postgresReturningRows<{ id: string }>(
    await databaseConnection().query(
      `UPDATE veritly_office_registration
...
async function attachHook(row: CleanupRow, webhook: string) {
  if (row.webhookId && row.webhookId !== webhook)
    throw new OfficeRegistrationCleanupError();
  const result = postgresReturningRows<{ id: string }>(
    await databaseConnection().query(
      `UPDATE veritly_office_registration
